...
private lateinit var albumLayoutManager: GridLayoutManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mediaAdapter = MediaGridAdapter(::openMedia, ::showActions)
        albumAdapter = AlbumAdapter {
            startActivity(Intent(this, AlbumActivity::class.java).apply {
                putExtra(AlbumActivity.EXTRA_ID, it.id)
                putExtra(AlbumActivity.EXTRA_NAME, it.name)
            })
        }
...
albumLayoutManager = GridLayoutManager(
    ...
    if (resources.configuration.smallestScreenWidthDp >= 600) 3 else 2
        ...
        binding.retryPermission.setOnClickListener { requestMediaPermission() }
        ...
        vm.images.observe(this) { if (currentTab == R.id.photos) submitMedia(it) }
        vm.videos.observe(this) { if (currentTab == R.id.videos) submitMedia(it) }
        vm.albums.observe(this) { if (currentTab == R.id.albums) submitAlbums(it) }
        updatePermissionUi()
    }

    override fun onResume() {
        super.onResume()
        if (hasMediaPermission()) vm.refresh()
    }

    private fun renderTab() {
        binding.emptyText.text = when (currentTab) {
            R.id.photos -> getString(R.string.no_photos)
            R.id.videos -> getString(R.string.no_videos)
            else -> getString(R.string.no_albums)
        }
        when (currentTab) {
            R.id.photos -> submitMedia(vm.images.value.orEmpty())
            R.id.videos -> submitMedia(vm.videos.value.orEmpty())
            else -> submitAlbums(vm.albums.value.orEmpty())
        }
    }
    ...
    private fun updatePermissionUi() {
        ...
        binding.permissionGroup.visibility = if (granted) android.view.View.GONE else android.view.View.VISIBLE
        binding.contentGroup.visibility = if (granted) android.view.View.VISIBLE else android.view.View.GONE
    ...
    private fun showActions(item: MediaItem) {
        val choices = arrayOf(
            ...
            if (item.isFavorite) getString(R.string.remove_favorite) else getString(R.string.add_favorite),
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(choices) { _, which ->
                when (which) {
                    0 -> share(item)
                    1 -> vm.toggleFavorite(item) { renderTab() }
                    2 -> MediaInfoDialog.show(this, item)
                    3 -> delete(item)
                }
            }.show()
    }
                ...
                runCatching {
                    if (Build.VERSION.SDK_INT >= 30) {
                        startIntentSenderForResult(
                            android.provider.MediaStore.createDeleteRequest(contentResolver, listOf(item.uri)).intentSender,
                            42, null, 0, 0, 0
                        )
                    } else contentResolver.delete(item.uri, null, null)
                }.onFailure { Snackbar.make(binding.root, R.string.delete_failed, Snackbar.LENGTH_SHORT).show() }
            }.show()
    }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode == 42) vm.refresh()
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.search) {
            SearchDialog.show(this, vm.images.value.orEmpty() + vm.videos.value.orEmpty(), ::openMedia)
            return true
        }
